 ...
 *
 * Same setup as testcube_minus75_protrusion_fuse_merge_repro (fuse the +X
 * protrusion, shifted -75mm in Y, onto the inner -Y wall), but merges with
 * the inner cube's BOTTOM panel (z=-75) instead of its TOP panel (z=+75) —
 * isolating whether the bend's fold DIRECTION (not just the bend-zone
 * Y-range fixed in Bug 5) is computed correctly for this mirrored case.
 */
...
describe('[repro] testcube.step: -75mm protrusion alignment (inner -Y wall), fuse, merge with inner BOTTOM', () => {
  ...
  it('+X protrusion moved -75mm in Y onto inner -Y wall, fused, merged with inner BOTTOM — must conserve volume, preserve panel shapes at the right coordinates, fold the correct direction (no gap), and export a real mesh', async () => {
    ...
    if (!innerMinusYWallEntry || !innerBottomEntry || !plusXProtrusionEntry) {
      console.warn('[minus75-bottom-repro] could not classify required panels:', {
        innerMinusYWall: !!innerMinusYWallEntry, innerBottom: !!innerBottomEntry, plusXProtrusion: !!plusXProtrusionEntry,
      });
      return;
    }
    ...
    console.log(`[minus75-bottom-repro] innerMinusYWall: ${fmt(innerMinusYWallBbox)}`);
    console.log(`[minus75-bottom-repro] innerBottom:     ${fmt(innerBottomBbox)}`);
    console.log(`[minus75-bottom-repro] protrusion:      ${fmt(plusXProtrusionBbox)}`);
    ...
    console.log(`[minus75-bottom-repro] pre-op volumes: wall=${wallMass.volume?.toFixed(1)} bottom=${bottomMass.volume?.toFixed(1)} prot=${protMass.volume?.toFixed(1)}`);
    ...
    console.log(`[minus75-bottom-repro] shifted protrusion: ${fmt(translatedBbox)}`);
    ...
    try {
      ...
      fuseError = err;
      console.log(`[minus75-bottom-repro] fuse_bodies threw: ${JSON.stringify(err, Object.getOwnPropertyNames(err as object))}`);
    }
    expect(fuseError, '[minus75-bottom-repro] fuse_bodies must not throw').toBeNull();
    if (!fused) return;
    ...
    console.log(`[minus75-bottom-repro] fused panel bbox: ${fmt(fusedBbox)} volume=${fusedMass.volume?.toFixed(1)}`);
    ...
    console.log(`[minus75-bottom-repro] fused flat: ${unfoldFused.flat_width_mm?.toFixed(1)} x ${unfoldFused.flat_height_mm?.toFixed(1)}mm`);
    ...
    try {
      ...
      mergeError = err;
      console.log(`[minus75-bottom-repro] merge threw: ${JSON.stringify(err, Object.getOwnPropertyNames(err as object))}`);
    }
    expect(mergeError, '[minus75-bottom-repro] merge_bodies_with_bend must not throw').toBeNull();
    if (!merged) return;
    ...
    console.log(`[minus75-bottom-repro] merged result bbox (solved): ${fmt(mergedBbox)}`);
    ...
    console.log(`[minus75-bottom-repro] merged flat: ${unfold.flat_width_mm?.toFixed(1)} x ${unfold.flat_height_mm?.toFixed(1)}mm`);
    ...
    if (unfold.dxf_content) {
      ...
      console.log(`[minus75-bottom-repro] merged DXF vertices (xMin=${xMin.toFixed(2)} xMax=${xMax.toFixed(2)} xMid=${xMid.toFixed(2)}):`);
      ...
      console.log(`[minus75-bottom-repro] low-X (A) Y-range: [${Math.min(...lowXYs).toFixed(2)}, ${Math.max(...lowXYs).toFixed(2)}] extent=${(Math.max(...lowXYs) - Math.min(...lowXYs)).toFixed(2)}`);
      console.log(`[minus75-bottom-repro] high-X (B) Y-range: [${Math.min(...highXYs).toFixed(2)}, ${Math.max(...highXYs).toFixed(2)}] extent=${(Math.max(...highXYs) - Math.min(...highXYs)).toFixed(2)}`);
    }
    ...
    try {
      ...
      console.log(`[minus75-bottom-repro] exportGlb succeeded, ${glb.length} bytes`);
    } catch (err) {
      glbError = err;
      console.log(`[minus75-bottom-repro] exportGlb threw: ${JSON.stringify(err, Object.getOwnPropertyNames(err as object))}`);
    }
    expect(glbError, '[minus75-bottom-repro] [BUG] exportGlb threw').toBeNull();
    ...
    console.log(`[minus75-bottom-repro] merged volume: actual=${mergedMass.volume?.toFixed(1)} expected≈${expectedTotalVolume.toFixed(1)} ratio=${totalRatio.toFixed(3)}`);
    expect(totalRatio, `[minus75-bottom-repro] [BUG] merged volume ratio=${totalRatio.toFixed(3)} — total volume not conserved`).toBeGreaterThan(0.9);
    expect(totalRatio, `[minus75-bottom-repro] [BUG] merged volume ratio=${totalRatio.toFixed(3)} — total volume not conserved`).toBeLessThan(1.1);
    ...
    console.log(`[minus75-bottom-repro] probe box bbox: ${fmt(probeBbox)}`);
    ...
    expect(intersected.solid_id, '[minus75-bottom-repro] [BUG] intersection with panel A\'s own probe region must not be empty').toBeDefined();
    ...
    console.log(`[minus75-bottom-repro] recovered panel-A-region bbox: ${fmt(intersectedBbox)} volume=${intersectedMass.volume?.toFixed(1)}`);

    const TOL_MM = 1.5;
    for (const k of ['x_min', 'x_max', 'y_min', 'y_max', 'z_min', 'z_max'] as const) {
      const delta = Math.abs(intersectedBbox[k] - fusedBbox[k]);
      expect(delta,
        `[minus75-bottom-repro] [BUG] recovered panel-A-region bbox.${k}=${intersectedBbox[k].toFixed(2)} doesn't match panel A's own pre-merge bbox.${k}=${fusedBbox[k].toFixed(2)} (Δ=${delta.toFixed(2)}mm)`)
        .toBeLessThanOrEqual(TOL_MM);
    }
    const panelARatio = intersectedMass.volume / fusedMass.volume;
    expect(panelARatio,
      `[minus75-bottom-repro] [BUG] recovered panel-A-region volume=${intersectedMass.volume?.toFixed(1)} doesn't match panel A's own pre-merge volume=${fusedMass.volume?.toFixed(1)} (ratio=${panelARatio.toFixed(3)})`)
    ...
    expect(cutResult.solid_id, '[minus75-bottom-repro] [BUG] panel B region (merged minus panel A probe) must not be empty').toBeDefined();
    ...
    console.log(`[minus75-bottom-repro] panel-B-region (innerBottom) bbox: ${fmt(panelBBbox)} volume=${panelBMass.volume?.toFixed(1)}`);
    console.log(`[minus75-bottom-repro] panel-B-region extents: x=${ext(panelBBbox, 'x').toFixed(2)} y=${ext(panelBBbox, 'y').toFixed(2)} z=${ext(panelBBbox, 'z').toFixed(2)}`);
    console.log(`[minus75-bottom-repro] innerBottom's OWN pre-merge extents: x=${ext(innerBottomBbox, 'x').toFixed(2)} y=${ext(innerBottomBbox, 'y').toFixed(2)} z=${ext(innerBottomBbox, 'z').toFixed(2)}`);
    console.log(`[minus75-bottom-repro] panel A region (fused wall+protrusion) bbox for reference: ${fmt(intersectedBbox)}`);
    ...
    expect(Math.abs(panelBExtentX - ext(innerBottomBbox, 'x')),
      `[minus75-bottom-repro] [BUG] panel B's (innerBottom's) region X-extent=${panelBExtentX.toFixed(2)}mm, expected≈${ext(innerBottomBbox, 'x').toFixed(2)}mm (its own unchanged width) — it appears to have inherited panel A's extra width instead of keeping its own`)
    ...
    expect(Math.abs(panelBBbox.x_min - innerBottomBbox.x_min),
      `[minus75-bottom-repro] [BUG] panel B's region x_min=${panelBBbox.x_min.toFixed(2)} doesn't match its own true x_min=${innerBottomBbox.x_min.toFixed(2)} — same width, but SHIFTED position`)
      .toBeLessThanOrEqual(3.0);
    expect(Math.abs(panelBBbox.x_max - innerBottomBbox.x_max),
      `[minus75-bottom-repro] [BUG] panel B's region x_max=${panelBBbox.x_max.toFixed(2)} doesn't match its own true x_max=${innerBottomBbox.x_max.toFixed(2)} — same width, but SHIFTED position`)
      .toBeLessThanOrEqual(3.0);
    ...
    console.log(`[minus75-bottom-repro] candidate seam gaps: Y=${gapY.toFixed(2)}/${gapYAlt.toFixed(2)} Z=${gapZ.toFixed(2)}/${gapZAlt.toFixed(2)} (expect ONE near 0, indicating where A and B actually meet)`);
  }, 60_000);
});
