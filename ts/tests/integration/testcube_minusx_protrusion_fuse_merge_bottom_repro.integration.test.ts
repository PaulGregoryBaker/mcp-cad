 ...
 *
 * User clarified the exact combination after two near-misses (-Y-wall and
 * +Y-wall, both with the +X protrusion, merged with BOTTOM — both passed
 * clean): "different protrusion, and different panel. Exact opposite sides
 * of the cube." This is the TRUE mirror of the original Bug-3/4/5 TOP-merge
 * repro on EVERY axis: the -X protrusion (not +X) shifted +75mm in Y onto
 * the inner +Y wall (not -Y), fused, then merged with the inner cube's
 * BOTTOM panel (not TOP).
 */
...
describe('[repro] testcube.step: -X protrusion +75mm alignment (inner +Y wall), fuse, merge with inner BOTTOM', () => {
  afterEach(async () => {
  ...
  it('-X protrusion moved +75mm in Y onto inner +Y wall, fused, merged with inner BOTTOM — must conserve volume, preserve panel shapes at the right coordinates, fold the correct direction (no gap), and export a real mesh', async () => {
    ...
    if (!innerPlusYWallEntry || !innerBottomEntry || !minusXProtrusionEntry) {
      console.warn('[minusx-bottom-repro] could not classify required panels:', {
        innerPlusYWall: !!innerPlusYWallEntry, innerBottom: !!innerBottomEntry, minusXProtrusion: !!minusXProtrusionEntry,
      });
      return;
    }
    ...
    console.log(`[minusx-bottom-repro] innerPlusYWall: ${fmt(innerPlusYWallBbox)}`);
    console.log(`[minusx-bottom-repro] innerBottom:     ${fmt(innerBottomBbox)}`);
    console.log(`[minusx-bottom-repro] protrusion:      ${fmt(minusXProtrusionBbox)}`);
    ...
    const protMass: any = await dispatchTool('mass_properties', { target: minusXProtrusion, properties: ['volume'] }, config);
    console.log(`[minusx-bottom-repro] pre-op volumes: wall=${wallMass.volume?.toFixed(1)} bottom=${bottomMass.volume?.toFixed(1)} prot=${protMass.volume?.toFixed(1)}`);

    const txn: any = await dispatchTool('begin_transaction', { label: 'minusx-bottom-repro' }, config);
    ...
    console.log(`[minusx-bottom-repro] shifted protrusion: ${fmt(translatedBbox)}`);
    ...
    try {
      ...
      fuseError = err;
      console.log(`[minusx-bottom-repro] fuse_bodies threw: ${JSON.stringify(err, Object.getOwnPropertyNames(err as object))}`);
    }
    expect(fuseError, '[minusx-bottom-repro] fuse_bodies must not throw').toBeNull();
    if (!fused) return;
    ...
    try {
      ...
      mergeError = err;
      console.log(`[minusx-bottom-repro] merge threw: ${JSON.stringify(err, Object.getOwnPropertyNames(err as object))}`);
    }
    expect(mergeError, '[minusx-bottom-repro] merge_bodies_with_bend must not throw').toBeNull();
    if (!merged) return;

    // RUN SOLVE GEOMETRY to reconstruct the real 3D assembly from the manufacturing graph.
    // The user has rent-first identified that the pre-solve mesh hid the reconstruction bugs!
    ...
    console.log(`[minusx-bottom-repro] merged result bbox (solved): ${fmt(mergedBbox)}`);
    ...
    if (unfold.dxf_content) {
      ...
      console.log(`[minusx-bottom-repro] merged DXF vertices (xMin=${xMin.toFixed(2)} xMax=${xMax.toFixed(2)} xMid=${xMid.toFixed(2)}):`);
      ...
      console.log(`[minusx-bottom-repro] low-X (A) Y-range: [${Math.min(...lowXYs).toFixed(2)}, ${Math.max(...lowXYs).toFixed(2)}] extent=${(Math.max(...lowXYs) - Math.min(...lowXYs)).toFixed(2)}`);
      console.log(`[minusx-bottom-repro] high-X (B) Y-range: [${Math.min(...highXYs).toFixed(2)}, ${Math.max(...highXYs).toFixed(2)}] extent=${(Math.max(...highXYs) - Math.min(...highXYs)).toFixed(2)}`);
    }
    ...
    try {
      ...
      console.log(`[minusx-bottom-repro] exportGlb succeeded, ${glb.length} bytes`);
    } catch (err) {
      glbError = err;
      console.log(`[minusx-bottom-repro] exportGlb threw: ${JSON.stringify(err, Object.getOwnPropertyNames(err as object))}`);
    }
    expect(glbError, '[minusx-bottom-repro] [BUG] exportGlb threw').toBeNull();
    ...
    console.log(`[minusx-bottom-repro] merged volume: actual=${mergedMass.volume?.toFixed(1)} expected≈${expectedTotalVolume.toFixed(1)} ratio=${totalRatio.toFixed(3)}`);
    expect(totalRatio, `[minusx-bottom-repro] [BUG] merged volume ratio=${totalRatio.toFixed(3)} — total volume not conserved`).toBeGreaterThan(0.9);
    expect(totalRatio, `[minusx-bottom-repro] [BUG] merged volume ratio=${totalRatio.toFixed(3)} — total volume not conserved`).toBeLessThan(1.1);
    ...
    console.log(`[minusx-bottom-repro] probe box bbox: ${fmt(probeBbox)}`);
    ...
    expect(intersected.solid_id, '[minusx-bottom-repro] [BUG] intersection with panel A\'s own probe region must not be empty').toBeDefined();
    ...
    console.log(`[minusx-bottom-repro] recovered panel-A-region bbox: ${fmt(intersectedBbox)} volume=${intersectedMass.volume?.toFixed(1)}`);

    const TOL_MM = 1.5;
    for (const k of ['x_min', 'x_max', 'y_min', 'y_max', 'z_min', 'z_max'] as const) {
      const delta = Math.abs(intersectedBbox[k] - fusedBbox[k]);
      expect(delta,
        `[minusx-bottom-repro] [BUG] recovered panel-A-region bbox.${k}=${intersectedBbox[k].toFixed(2)} doesn't match panel A's own pre-merge bbox.${k}=${fusedBbox[k].toFixed(2)} (Δ=${delta.toFixed(2)}mm)`)
        .toBeLessThanOrEqual(TOL_MM);
    }
    const panelARatio = intersectedMass.volume / fusedMass.volume;
    expect(panelARatio,
      `[minusx-bottom-repro] [BUG] recovered panel-A-region volume=${intersectedMass.volume?.toFixed(1)} doesn't match panel A's own pre-merge volume=${fusedMass.volume?.toFixed(1)} (ratio=${panelARatio.toFixed(3)})`)
    ...
    expect(cutResult.solid_id, '[minusx-bottom-repro] [BUG] panel B region (merged minus panel A probe) must not be empty').toBeDefined();
    ...
    console.log(`[minusx-bottom-repro] panel-B-region (innerBottom) bbox: ${fmt(panelBBbox)} volume=${panelBMass.volume?.toFixed(1)}`);
    console.log(`[minusx-bottom-repro] panel-B-region extents: x=${ext(panelBBbox, 'x').toFixed(2)} y=${ext(panelBBbox, 'y').toFixed(2)} z=${ext(panelBBbox, 'z').toFixed(2)}`);
    console.log(`[minusx-bottom-repro] innerBottom's OWN pre-merge extents: x=${ext(innerBottomBbox, 'x').toFixed(2)} y=${ext(innerBottomBbox, 'y').toFixed(2)} z=${ext(innerBottomBbox, 'z').toFixed(2)}`);
    console.log(`[minusx-bottom-repro] panel A region (fused wall+protrusion) bbox for reference: ${fmt(intersectedBbox)}`);
    ...
    expect(Math.abs(panelBExtentX - ext(innerBottomBbox, 'x')),
      `[minusx-bottom-repro] [BUG] panel B's (innerBottom's) region X-extent=${panelBExtentX.toFixed(2)}mm, expected≈${ext(innerBottomBbox, 'x').toFixed(2)}mm (its own unchanged width) — it appears to have inherited panel A's extra width instead of keeping its own`)
      .toBeLessThanOrEqual(3.0);

    // POSITION check (not just extent/magnitude): the bend axis here is
    // parallel to world-X (rotating panel B about an axis parallel to X
    // cannot change X-coordinates), so panel B's X-RANGE must exactly match
    // its own pre-merge world X-range — not just have the same WIDTH. A
    // same-width-but-shifted region (the actual bug found via manual
    // diagnostics: panel B's region lands at x[-98,52] instead of its true
    // x[-75,75]) has the same extent and would incorrectly pass an
    // extent-only check — this is what actually catches it.
    expect(Math.abs(panelBBbox.x_min - innerBottomBbox.x_min),
      `[minusx-bottom-repro] [BUG] panel B's region x_min=${panelBBbox.x_min.toFixed(2)} doesn't match its own true x_min=${innerBottomBbox.x_min.toFixed(2)} — same width, but SHIFTED position (likely shifted by the appendage's width along the seam axis)`)
      .toBeLessThanOrEqual(3.0);
    expect(Math.abs(panelBBbox.x_max - innerBottomBbox.x_max),
      `[minusx-bottom-repro] [BUG] panel B's region x_max=${panelBBbox.x_max.toFixed(2)} doesn't match its own true x_max=${innerBottomBbox.x_max.toFixed(2)} — same width, but SHIFTED position (likely shifted by the appendage's width along the seam axis)`)
      .toBeLessThanOrEqual(3.0);
    ...
    console.log(`[minusx-bottom-repro] candidate seam gaps: Y=${gapY.toFixed(2)}/${gapYAlt.toFixed(2)} Z=${gapZ.toFixed(2)}/${gapZAlt.toFixed(2)} (expect ONE near 0, indicating where A and B actually meet)`);
  }, 60_000);
});
