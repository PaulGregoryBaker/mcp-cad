...
| [18-stack-evaluation-plan.md](18-stack-evaluation-plan.md) | **Phase 3 — stack evaluation plan**: unbundles kernel/orchestration-language/MCP-layer-language as separable decisions; boundary-crossing resource lifecycle (N13, not raw speed — confirmed observed failure mode) elevated to a first-class, spike-validated requirement, secondary in priority to functional correctness; candidates incl. status quo (no new diagnostic spike — its own history is the evidence), Rust+OCCT hybrid, pure Rust, and a port-split hybrid; evaluation via time-boxed spikes (scope-scaled ceilings, attempt-budgeted) before ADRs. All 3 open questions answered/ratified 2026-07-20 — a plan, not a decision; spikes not yet run. |
