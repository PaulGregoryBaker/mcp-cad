...
| [17-numerical-policy.md](17-numerical-policy.md) | **Phase 2.4, cont'd — numerical policy module spec**: the N11-vs-policy boundary rule (project tolerance budgets vs. fixed numerical-robustness constants), fixed constants table, comparison helpers, degeneracy-is-reported-never-repaired discipline. Awaiting review. |
| [18-stack-evaluation-plan.md](18-stack-evaluation-plan.md) | **Phase 3 — stack evaluation plan**: unbundles kernel/orchestration-language/MCP-layer-language as separable decisions; boundary-crossing performance (TS↔C++ and controller↔MCP) elevated to a first-class, spike-validated requirement; candidates incl. status quo, Rust+OCCT hybrid, pure Rust, and a port-split hybrid; evaluation via time-boxed spikes before ADRs. A plan, not a decision — awaiting Paul's direction. |
